...
exports.createGRangesListTools = createGRangesListTools;
exports.handleGRangesListTool = handleGRangesListTool;
function createGRangesListTools(rBridge) {
    return [
        {
...
async function handleGRangesListTool(toolName, args, rBridge) {
    ...
    switch (toolName) {
        ...
        case 'gutils_grl_unlist': {
            ...
            if (args.keep_names !== undefined)
                cmd += `, keep.names = ${args.keep_names ? 'TRUE' : 'FALSE'}`;
            cmd += ')';
            const result = await rBridge.executeRCommand(cmd);
            return rBridge.parseROutput(result);
        }
        ...
        case 'gutils_grl_eval': {
            ...
            if (args.condition)
                cmd += `, condition = expression(${args.condition})`;
            cmd += ')';
            const result = await rBridge.executeRCommand(cmd);
            return rBridge.parseROutput(result);
        }
        ...
        case 'gutils_grl_bind': {
            const grlObjects = args.lists.map((grl) => rBridge.formatRObject({ type: 'GRangesList', data: grl })).join(', ');
...
//# sourceMappingURL=grangeslist-operations.js.map