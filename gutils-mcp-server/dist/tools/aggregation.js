...
exports.createAggregationTools = createAggregationTools;
exports.handleAggregationTool = handleAggregationTool;
function createAggregationTools(rBridge) {
    return [
        {
            name: 'gutils_gr_val',
...
async function handleAggregationTool(toolName, args, rBridge) {
        ...
        case 'gutils_gr_val': {
            ...
            if (args.val) {
                if (Array.isArray(args.val)) {
                    const valStr = args.val.map((v) => `"${v}"`).join(', ');
                    cmd += `, val = c(${valStr})`;
                }
                else {
                    cmd += `, val = "${args.val}"`;
            ...
            if (args.FUN)
                cmd += `, FUN = ${args.FUN}`;
            if (args.na_rm !== undefined)
                cmd += `, na.rm = ${args.na_rm ? 'TRUE' : 'FALSE'}`;
            if (args.weighted !== undefined)
                cmd += `, weighted = ${args.weighted ? 'TRUE' : 'FALSE'}`;
            if (args.mean !== undefined)
                cmd += `, mean = ${args.mean ? 'TRUE' : 'FALSE'}`;
        ...
        case 'gutils_gr_sum': {
            ...
            if (args.by) {
                if (Array.isArray(args.by)) {
                    const byStr = args.by.map((b) => `"${b}"`).join(', ');
                    cmd += `, by = c(${byStr})`;
                }
                else {
                    cmd += `, by = "${args.by}"`;
