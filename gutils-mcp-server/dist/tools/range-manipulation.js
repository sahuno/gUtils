...
exports.createRangeManipulationTools = createRangeManipulationTools;
exports.handleRangeManipulationTool = handleRangeManipulationTool;
function createRangeManipulationTools(rBridge) {
    return [
        {
...
async function handleRangeManipulationTool(toolName, args, rBridge) {
    ...
    switch (toolName) {
        case 'gutils_gr_start': {
            ...
            if (args.force !== undefined)
                cmd += `, force = ${args.force ? 'TRUE' : 'FALSE'}`;
        ...
        case 'gutils_gr_tile': {
            ...
            if (args.width !== undefined)
                cmd += `, width = ${args.width}`;
            if (args.stranded !== undefined)
                cmd += `, stranded = ${args.stranded ? 'TRUE' : 'FALSE'}`;
            cmd += ')';
            const result = await rBridge.executeRCommand(cmd);
            return rBridge.parseROutput(result);
        }
        ...
        case 'gutils_gr_sample': {
            ...
            if (args.replace !== undefined)
                cmd += `, replace = ${args.replace ? 'TRUE' : 'FALSE'}`;
            cmd += ')';
            const result = await rBridge.executeRCommand(cmd);
            return rBridge.parseROutput(result);
        }
...
//# sourceMappingURL=range-manipulation.js.map