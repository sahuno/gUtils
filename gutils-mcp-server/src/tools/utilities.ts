...
export function createUtilityTools(rBridge: RBridge): Tool[] {
  return [
    {
...
export async function handleUtilityTool(
  ...
  switch (toolName) {
    case 'gutils_hg_seqlengths': {
      ...
      if (args.genome) cmd += `genome = "${args.genome}"`;
      if (args.chr !== undefined) cmd += `, chr = ${args.chr ? 'TRUE' : 'FALSE'}`;
      if (args.include_junk !== undefined) cmd += `, include.junk = ${args.include_junk ? 'TRUE' : 'FALSE'}`;
    ...
    case 'gutils_grbind': {
      const grObjects = args.ranges_list.map((gr: any) => 
        rBridge.formatRObject({ type: 'GRanges', data: gr })
      ).join(', ');
    ...
    case 'gutils_rrbind': {
      const tableObjects = args.tables.map((table: any) => 
        rBridge.formatRObject(table)
      ).join(', ');
      
      let cmd = `rrbind(list(${tableObjects})`;
      
      if (args.fill !== undefined) cmd += `, fill = ${args.fill ? 'TRUE' : 'FALSE'}`;
