...
import { GenomicRange, DataTable } from '../types/genomic';

export function createDataConversionTools(rBridge: RBridge): Tool[] {
  return [
    {
      name: 'gutils_gr2dt',
...
export async function handleDataConversionTool(
  toolName: string,
  args: any,
  rBridge: RBridge
): Promise<any> {
  ...
  switch (toolName) {
    ...
    case 'gutils_parse_gr': {
      const coordsStr = args.coordinates.map((c: string) => `"${c}"`).join(', ');
    ...
    case 'gutils_si2gr': {
      ...
      if (args.chromosomes) {
        const chrStr = args.chromosomes.map((c: string) => `"${c}"`).join(', ');
        cmd = `
